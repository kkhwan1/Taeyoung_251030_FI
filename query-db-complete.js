...
async function queryDatabase() {
  ...
  try {
    ...
    console.log('\n8. PRICE MASTER SAMPLE (10 records)\n');
    ...
    if (pricesError) {
      console.log('ERROR:', pricesError.message);
    } else {
      console.log(JSON.stringify(prices, null, 2));
    }

    console.log('\n9. INVENTORY TRANSACTIONS BY TYPE\n');
    const { data: tranTypes, error: tranTypesError } = await supabase
      .from('inventory_transactions')
      .select('transaction_type');
    
    if (tranTypesError) {
      console.log('ERROR:', tranTypesError.message);
    } else {
      const typeCounts = {};
      tranTypes.forEach(t => {
        const type = t.transaction_type || 'Uncategorized';
        typeCounts[type] = (typeCounts[type] || 0) + 1;
      });
      console.log(JSON.stringify(typeCounts, null, 2));
    }
