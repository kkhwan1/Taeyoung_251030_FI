-- Summary of Excel vs Database Price Comparison
WITH excel_prices AS (
  ...
  FROM (VALUES
    ...
    ('81836-R0500', 462.51, '에이오에스'),
    ('81846-R0500', 462.51, '에이오에스'),
    ('81836-R0600', 575.42, '에이오에스'),
    ('81846-R0600', 575.42, '에이오에스'),
    ...
    ('84191-A7500', 38, '호원사급'),
    ('81763-1Y000', 179.092, '호원사급'),
    ('81838-R0600', 320, '호원사급'),
    ('13905-06000', 11.4, '호원사급'),
...
SELECT
  status,
  COUNT(*) as count,
  ROUND(AVG(excel_price), 2) as avg_excel_price
FROM comparison
GROUP BY status
ORDER BY
  CASE status
    WHEN 'NOT_IN_DB' THEN 1
    WHEN 'DB_PRICE_NULL' THEN 2
