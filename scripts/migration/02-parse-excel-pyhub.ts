/**
 * Phase 2: Excel 파일 파싱 (pyhub MCP 사용)
 *
 * 열려있는 Excel 파일에서 직접 데이터를 읽어서 JSON으로 변환합니다.
 * - 태창금속 BOM.xlsx → parsed-bom.json
 * - 09월 원자재 수불관리.xlsx → parsed-inventory.json
 * - 2025년 9월 종합관리 SHEET.xlsx → parsed-comprehensive.json
 * - 2025년 9월 매입매출 보고현황.xlsx → parsed-purchase-sales.json
 *
 * 실행: npx tsx scripts/migration/02-parse-excel-pyhub.ts
 */
...
/**
 * 1. BOM Excel 파싱 - 모든 시트
 * 시트: 대우공업, 풍기산업, 다인, 호원오토, 인알파코리아 (최신단가 제외)
 * 헤더: 6행
 * 구조: 좌측(납품정보) + 우측(구매정보)
 */
async function parseBomExcel(logger: ReturnType<typeof createLogger>): Promise<ParseResult<BomExcelRow>> {
  logger.log('📄 BOM 파일 파싱 시작 (pyhub MCP) - 5개 시트', 'info');
  ...
  try {
    ...
    for (const sheetName of sheets) {
      ...
      const csvData = await mcp__pyhub_mcptools__excel_get_values({
        ...
        sheet_range: 'A7:P1000', // 7행부터, P열까지
        value_type: 'values'
      });
...
async function parseComprehensiveExcel(logger: ReturnType<typeof createLogger>): Promise<ParseResult<ComprehensiveExcelRow>> {
  logger.log('📄 종합관리 SHEET 파일 파싱 시작 (pyhub MCP)', 'info');
...
async function parsePurchaseSalesExcel(logger: ReturnType<typeof createLogger>): Promise<ParseResult<PurchaseSalesExcelRow>> {
  logger.log('📄 매입매출 파일 파싱 시작 (pyhub MCP)', 'info');
  ...
  try {
    ...
    const csvData = await mcp__pyhub_mcptools__excel_get_values({
      ...
      sheet_name: '정리',
      sheet_range: 'A5:C300', // 5행부터, 업체/차종/첫날 데이터만
      value_type: 'values'
    });
    ...
    rows.forEach((row, index) => {
      try {
        ...
        const 품목코드 = String(row[2] || '').trim(); // 차종을 품목코드로 사용
        ...
        if (!거래처명 || !품목코드) {
          throw new Error('필수 필드 누락: 거래처명, 품목코드');
        }

        const purchaseSalesRow: PurchaseSalesExcelRow = {
          거래일자: '2025-09-01', // 기본 날짜
          거래처명: 거래처명,
          품목코드: 품목코드,
          품목명: 품목코드, // 차종을 품명으로도 사용
          규격: '',
          수량: 1,
          단가: 0,
          금액: 0,
